...
namespace Template.Utilities
{
    public class ServerHelper
    {
        ...
        public async Task SendAnnouncementAsync(IGuild guild, string title, string description)
        {
            var channelId = await _servers.GetAnnouncementChannelAsync(guild.Id);
            if (channelId == 0)
                return;
            ...
            await fetchedChannel.SendAnnouncementAsync(title, description);
        }
        ...
        public async Task GetAnnouncementAsync(IGuild guild)
        {
            var channelId = await _servers.GetLogsAsync(guild.Id);
            if (channelId == 0)
                return;
            ...
            await _servers.GetAnnouncementsAsync(guild.Id, channelId);
            return;
        }

        public async Task GetAnnouncementChannelAsyc(IGuild guild)
        {
            var channelId = await _servers.GetLogsAsync(guild.Id);
            if (channelId == 0)
                return;
            ...
            await _servers.GetAnnouncementChannelAsync(guild.Id);
            return;
        }
