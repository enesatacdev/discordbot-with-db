...
namespace Infrastructure
{
    public class Servers
    {
        ...
        public Servers(DataContext context)
        {
            _context = context;
        }
        ...
        public async Task GetAnnouncementsAsync(ulong id, ulong channelId)
        {
            var server = await _context.Servers.FindAsync(id);
            server.Announcements = channelId;
            await _context.SaveChangesAsync();
        }

        public async Task<ulong> GetAnnouncementChannelAsync(ulong id)
        {
            var server = await _context.Servers.FindAsync(id);
            ulong announcementChannel = server.Announcements;
            return await Task.FromResult(announcementChannel);
        }
    }
}
